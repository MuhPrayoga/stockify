document.addEventListener("DOMContentLoaded", async () => {
    ...
    const opRes = await fetch("/stock-opname", {
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
        },
    ...
    opResult.data.forEach((o) => {
        ...
        opnameTable.innerHTML += `
            <tr class="hover:bg-gray-50 transition">
                <td class="px-6 py-4 font-semibold">${o.product?.name ?? "-"}</td>
                <td class="px-6 py-4">${o.system_stock}</td>
                <td class="px-6 py-4">${o.physical_stock}</td>
                <td class="px-6 py-4 font-bold ${diffColor}">
                    ${o.difference}
                </td>
                <td class="px-6 py-4">${o.notes ?? "-"}</td>
                <td class="px-6 py-4">${o.checker?.name ?? "-"}</td>
                <td class="px-6 py-4">${o.checked_at}</td>
            </tr>
        `;
    });
});
